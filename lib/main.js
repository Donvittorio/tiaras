  ...
  tiaras.provider('Tiaras', function() {
    ...
    this.$get = function($http, $q, $injector, ModelStore) {
      ...
      var TiarasEntity = function(model, data) {
        ...
        var define_belongs_to = function(entity, rel) {
          var value = {};
          Object.defineProperty(entity, rel.name_sing, {
            configurable: true,
            get: function() {
              var res = value;

              res.get = function() {
                var url = baseUrl + '/' + rel.name_mult + '/' + entity.id;
            ...
            set: function(newVal) {
              console.log("hey");
              $.extend(entity[rel.name_sing], newVal);
            }
          });
        };
