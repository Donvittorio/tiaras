(function() {
  ...
  tiaras.provider('Tiaras', function() {
    ...
    this.$get = function($http, $q, $injector, ModelStore) {
      ...
      function TiarasCollection(model, data) {
        var arr = [];
        //REVIEW: using __proto__ is probably not a good idea as it is non-standard
        arr.__proto__ = TiarasCollection.prototype;
        for (var i = 0; i < data.length; i++) {
          arr[i] = new TiarasEntity(model, data[i]);
        }
        return arr;
      }
      TiarasCollection.prototype = new Array;
      ...
      var TiarasEntity = function(model, data) {
        ...
        var define_belongs_to = function(entity, relation) {
          ...
          //        the only decent use for it is the setter, which will never
          //        overwrite the .get function
          //
      ...
      var model = function(modelname, names, config) {
        ...
        tiara_model.all = {
          ...
          get: function() {
            ...
            $http.get(this.url).success(function(data) {
              deferred.resolve(new TiarasCollection(tiara_model, data));
            });
